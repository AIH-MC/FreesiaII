...
public class RealPlayerYsmPacketProxyImpl extends YsmPacketProxyLayer {
    ...
    public ProxyComputeResult processS2C(Key key, ByteBuf copiedPacketData) {
        ...
        if (packetId == YsmProtocolMetaFile
            ...
            Freesia.LOGGER.info("Replying ysm client with server version " + backendVersion + ".Can switch model? : "
                    + canSwitchModel);
            Freesia.LOGGER.info("Replying ysm client with server version " + backendVersion + ".Can upload model? : "
                    + canUploadModel);
    ...
    private static boolean isAnyWorkerModelUploadAllowed() {
        return Freesia.registedWorkers.values().stream()
                .anyMatch(com.nguyendevs.freesia.waterfall.network.backend.MasterServerMessageHandler::isAllowModelUpload);
    }
