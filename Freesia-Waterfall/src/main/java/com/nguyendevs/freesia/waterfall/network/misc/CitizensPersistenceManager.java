...
public class CitizensPersistenceManager {
    ...
    public Map<String, byte[]> loadModelBinaryCache() {
        ...
        try (DataInputStream in = new DataInputStream(new BufferedInputStream(new FileInputStream(MODEL_CACHE_FILE)))) {
            ...
            Freesia.LOGGER.info("[Citizens] Loaded " + cache.size() + " model binary entries from cache");
        } catch (Exception e) {
            Freesia.LOGGER.warning("[Citizens] Failed to load citizens_model_cache.dat: " + e.getMessage());
        }
        return cache;
    }
